...
import app.lightai.data.AppModel
...
import app.lightai.databinding.FragmentHomeBinding
import app.lightai.helper.*
import app.lightai.helper.LightAINotificationListener
import app.lightai.listener.SwipeTouchListener
...
class HomeFragment :
    ...
    private fun observeNotificationChanges() {
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            LightAINotificationListener.changeVersion.collect {
                val current = LightAINotificationListener.getActiveNotificationPackages()
    ...
    private fun handleOtherAction(action: Action) {
        when (action) {
            ...
            Action.GoBack -> {
                initActionService(requireContext())?.goBack()
            }
            ...
            Action.BrightnessUp -> adjustBrightness(+0.1f)
    ...
    private fun openAssistant() {
        ...
        if (intent.resolveActivity(ctx.packageManager) != null) {
            ...
            Toast.makeText(ctx, R.string.toast_no_assistant_installed, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun adjustBrightness(delta: Float) {
        ...
        if (!android.provider.Settings.System.canWrite(ctx)) {
            // Ask user to grant WRITE_SETTINGS — open the system page
            val intent =
                Intent(android.provider.Settings.ACTION_MANAGE_WRITE_SETTINGS).apply {
            ...
            try {
                ...
                Toast.makeText(ctx, R.string.toast_grant_write_settings, Toast.LENGTH_SHORT).show()
            }
            return
        }
        val cr = ctx.contentResolver
        val max = 255
        val current = android.provider.Settings.System.getInt(cr, android.provider.Settings.System.SCREEN_BRIGHTNESS, max / 2)
        val deltaInt = (delta * max).toInt()
        val next = (current + deltaInt).coerceIn(1, max)
        android.provider.Settings.System.putInt(cr, android.provider.Settings.System.SCREEN_BRIGHTNESS, next)
    }
    ...
    private fun getAppDisplayName(appModel: AppModel): String {
        ...
        val packagesWithNotifications = LightAINotificationListener.getActiveNotificationPackages()
